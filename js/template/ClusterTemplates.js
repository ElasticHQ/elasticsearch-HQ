...
clusterTemplate.HealthDescribe = [
    ...
    '</div></div>',
    '<div class="span2">',
    '<div class="well stat-box"><span class="stat-detail"><%- numeral(indices.store.size_in_bytes).format("0.0b") %></span><span>Total Size</span>',
    '</div></div>',
    '</div>',
    '<div class="clear"></div>',
    '</div>',
    ...
    '</td><td><%- numeral(index.total.docs.count).format("0,0") %></td><td><%- numeral(index.total.store.size_in_bytes).format("0.0b")  %></td><td><%- numeral(index.numshards).format("0,0") %></td><td><%- numeral(index.numreplicas).format("0,0") %></td><td><%- index.status %></td></tr>',
    '<% }); %>',
    '</tbody>',
    '</table>',
    '<% } %>',
