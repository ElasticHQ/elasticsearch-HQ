...
nodeTemplate.diagnostics = [
    ...
    '<table class="table table-condensed table-bordered nodestable"  style="overflow-x: auto;width: auto;" >',
    ...
    '<td rel="popRight" data-trigger="hover" data-container="body" data-html="true" data-placement="top" data-title="<b>How this Field is Calculated</b>" data-content="<%- makeDiagnosticsPopOver(node, rule) %>" style="white-space: nowrap; text-align: right;" class="<%- calculateCellClass(node, rule) %>">',
    ...
    '</td>',
    '<% _.each(nodes, function(node, k) { %>',
    '<td rel="popRight" data-trigger="hover" data-container="body" data-html="true" data-placement="top" data-title="<b>How this Field is Calculated</b>" data-content="<%- makeDiagnosticsPopOver(node, rule) %>" style="white-space: nowrap; text-align: right;" class="<%- calculateCellClass(node, rule) %>">', '<%- calculateRuleValue(node, rule.value, rule.unit, rule.format) %>',
    '</td>',
    '<% }); %>',
    '</tr>',
    '<% }); %>',
    ...
    '</td>',
    '<% _.each(nodes, function(node, k) { %>',
    '<td rel="popRight" data-trigger="hover" data-container="body" data-html="true" data-placement="top" data-title="<b>About this Field</b>" data-content="<%- makeDiagnosticsPopOver(node, rule) %>" style="white-space: nowrap; text-align: right;" class="<%- calculateCellClass(node, rule) %>">',
    '<%- calculateRuleValue(node, rule.value, rule.unit, rule.format) %>',
    '</td>',
    '<% }); %>',
    '</tr>',
...
nodeTemplate.nodeInfo = [
    ...
    '<div class="well">',
    '<div class="pull-left"><button id="refreshNodeInfo" class="btn btn-mini"  rel="tipRight" data-placement="bottom" data-html="true" data-title="Refreshing every 10 seconds.<br/>Click to Force Refresh."><i class="icon-refresh"></i> <%- lastUpdateTime %></button></div>',
